...
|---|-------|--------|----------|
| 1 | An academic writer exports an IEEE paper and gets a `.tex` file that `\documentclass{IEEEtran}` with Scriveno's title/authors/abstract/keywords pulled from project metadata | ✓ VERIFIED | `scriveno-ieee.latex` exists (77 lines), contains `\documentclass{IEEEtran}` with classoption pass-through, `$title$`/`$author$`/`$if(abstract)$`/`$if(keywords)$` all present; STEP 4 of `build-print.md` invokes `pandoc -o paper-ieee.tex --template=...` with no `--pdf-engine` |
...
| 8 | `scriveno-lncs.latex` contains splncs04 bib style comment and Springer download page URL | ✓ VERIFIED | Lines 72-77 contain `% \bibliographystyle{splncs04}` and `https://www.springer.com/gp/computer-science/lncs/conference-proceedings-guidelines` |
...
|----------|----------|--------|---------|
| `data/export-templates/scriveno-ieee.latex` | IEEEtran Pandoc wrapper, 60+ lines | ✓ VERIFIED | 77 lines, `\documentclass{IEEEtran}`, all boilerplate present |
| `data/export-templates/scriveno-acm.latex` | acmart Pandoc wrapper, 60+ lines | ✓ VERIFIED | 67 lines, `\documentclass{acmart}`, all boilerplate present |
| `data/export-templates/scriveno-lncs.latex` | llncs Pandoc wrapper, 60+ lines | ✓ VERIFIED | 79 lines, `\documentclass{llncs}`, splncs04 + Springer URL |
| `data/export-templates/scriveno-elsevier.latex` | elsarticle Pandoc wrapper, 60+ lines | ✓ VERIFIED | 76 lines, `\documentclass{elsarticle}`, all boilerplate present |
| `data/export-templates/scriveno-apa7.latex` | apa7 Pandoc wrapper, 60+ lines | ✓ VERIFIED | 83 lines, `\documentclass{apa7}`, biblatex in commented advisory only |
...
|------|-----|-----|--------|---------|
| `build-print.md` STEP 1.8 | `data/export-templates/scriveno-{platform}.latex` | `LATEX_TEMPLATE` path table | ✓ WIRED | Lines 178-185 map all 5 slugs to template paths |
...
| `scriveno-lncs.latex` | Springer author resources (advisory comment) | URL in bibliography comment block | ✓ WIRED | Line 77: `https://www.springer.com/gp/...` |
