...
describe('Phase 33: TRAD-01 all 10 tradition manifests are complete and non-placeholder', () => {
  ...
  it('templates/sacred/README.md describes shipped active profiles, not placeholder profiles -- TRAD-01', () => {
    const content = readFile(path.join(SACRED_DIR, 'README.md'));
    assert.ok(content !== null, 'templates/sacred/README.md must exist -- TRAD-01');
    assert.ok(content.includes('Currently shipped traditions'), 'sacred README should describe currently shipped traditions -- TRAD-01');
    assert.ok(!content.includes('All 10 ship with `status: placeholder`'), 'sacred README must not claim shipped profiles are placeholders -- TRAD-01');
  });
});
