...
describe('authenticity-check diagnostic discipline', () => {
  ...
  it('release metadata is aligned on 2.0.5', () => {
    ...
    assert.equal(cfg.scriveno_version, '2.0.5');
    assert.equal(constraints.version, '2.0.5');
    assert.match(read('CHANGELOG.md'), /## 2\.0\.5 - /);
    assert.match(read('docs/release-notes.md'), /## 2\.0\.5 - /);
  });
});
