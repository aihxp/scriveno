...
- `templates/RELATIONSHIPS.md` -- derived pairwise relationship-map scaffold: the openable file showing every character pairing (including pairs marked `none`) and surfacing undefined pairs to explore. Rebuilt from the relationship sections of `CHARACTERS.md`; auto-regenerated on `/scr:new-character`, `/scr:character-touch`, `/scr:relationship-map --edit`, `/scr:save`, and `/scr:scan --fix`. Rendered live by `/scr:relationship-map`.
- `templates/CONFLICTS.md` -- derived conflict-map scaffold: the openable file showing the central conflict and every character pairing (including pairs marked `no conflict`). Rebuilt from `WORK.md` and `CHARACTERS.md`; auto-regenerated on `/scr:new-character`, `/scr:character-touch`, `/scr:save`, and `/scr:scan --fix`. A neutral derived file (same name across work types).
...
- `docs/relationships-protocol.md` -- canonical contract for the derived `.manuscript/RELATIONSHIPS.md` pairwise map: derivation from `CHARACTERS.md`, the complete-matrix rule (`none` is a recorded value, not a blank), undefined-pair surfacing, and the regeneration points.
